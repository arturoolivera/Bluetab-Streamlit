...
def load_predictions():
    ...
    df = pd.read_csv('predicted_data_2.csv', parse_dates=True, index_col=0)
    return df
...
def plot_predictions(df, start_date, end_date):
    ...
    fig.add_trace(go.Scatter(x=filtered_df.index, y=filtered_df['predictions'], mode='lines', name='Predicted Price'))
    ...
    fig.update_yaxes(range=[0, max(filtered_df[['predictions', 'actual']].max().max(), 0)])  # Set y-axis range starting from 0
    return fig

def plot_predictions2(df, start_date, end_date):
    # Filter dataframe based on selected date range
    start_datetime = pd.to_datetime(start_date)
    end_datetime = pd.to_datetime(end_date)
    # Ensure the time component is set to start of the day for comparison
    ...
    fig.update_layout(title='Energy Price Predictions', xaxis_title='Date', yaxis_title='Price', legend_title='Legend')
    fig.update_yaxes(range=[0, max(filtered_df[['Middle', 'Upper', 'Lower']].max().max(), 0)])  # Set y-axis range starting from 0
    return fig
...
def calculate_metrics(df, start_date, end_date):
    ...
    mask = (df.index >= start_datetime) & (df.index <= end_datetime)
    filtered_df = df.loc[mask]
    # Calculate MAPE and MAE for the entire DataFrame
    mape = 100 - ((abs(filtered_df['predictions'] - filtered_df['actual']) / filtered_df['actual']).mean() * 100)
    mae = abs(filtered_df['predictions'] - filtered_df['actual']).mean()
    return mape, mae
...
def plot_adjusted_predictions_with_granularity(df, granularity):
    ...
    fig = plot_predictions2(agg_df, agg_df.index.min(), agg_df.index.max())  # Adjusted to use the whole range of agg_df
...
def show():
    ...
    mape, mae = calculate_metrics(df, start_date1, end_date1)
    
    st.write(f"Accuracy (1 - MAPE) for selected range: {mape:.2f}%")
    st.write(f"MAE for selected range: {mae:.2f}")
